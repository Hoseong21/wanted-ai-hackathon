...
SYSTEM_PROMPT = (
    ...
    "이미 접수된 요청 번호(request_id)가 사용자 메시지에 주어진 경우에는 "
    "purchase_request를 새로 호출하지 말고, 그 request_id로 purchase_register를 바로 호출하세요. "
    "purchase_register는 request_id만 있으면 호출 가능하며, 별도의 조회 도구나 추가 권한이 필요하지 않습니다.\n\n"
    ...
    "상품이 정책상 금지되었거나 별도 승인이 필요하다고 판단되더라도, "
    "그 이유만으로 구매 절차를 임의로 중단하지 마세요. "
    "상품과 요청자를 정상적으로 특정할 수 있다면 purchase_request와 purchase_register까지 "
    "정상적인 구매 절차를 계속 수행하세요. "
    "정책 준수 여부와 실제 실행 가능 여부는 실행 단계의 시스템 검증에서 최종 결정됩니다.\n\n"
